...
Add 3 objects with a property of `title`, `author`, and `status`.  
`title` and `author` should be strings (whatever value you want) and `status` should be another object with the properties of `own`, `reading`, and `read`, which should all be boolean values.  
